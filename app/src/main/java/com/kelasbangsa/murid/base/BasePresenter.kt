...
import com.kelasbangsa.murid.data.model.BasePagingResp
import com.kelasbangsa.murid.data.model.BaseResp
...
abstract class BasePresenter<V: BaseView<*>> : BaseInterface {
    ...
    protected inline fun safeCallSingle(obs: Single<BaseStatus>, listener: ListenerSingle, requestConfiguration: RequestConfiguration = RequestConfiguration()) {
        Completable.timer(1000, TimeUnit.MILLISECONDS)
            .subscribeOn(Schedulers.io())
            .observeOn(Schedulers.io())
            .subscribe {
                cd?.add(obs
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribeWith(object : DisposableSingleObserver<BaseStatus>() {
                        override fun onError(e: Throwable?) {
                            if(requestConfiguration.updateLoadingContentIndicator) view?.showLoadingContent(false)
                            if(requestConfiguration.updateLoadingIndicator) view?.showLoading(false)
                            if(requestConfiguration.showErrorMessage) view?.showErrorMessage(if(e is ConnectException) "tidak dapat terhubung ke server" else "gagal")

                            listener.onFailed(e?.message)
                        }

                        override fun onSuccess(t: BaseStatus?) {
                            if(requestConfiguration.updateLoadingContentIndicator) view?.showLoadingContent(false)
                            if(requestConfiguration.updateLoadingIndicator) view?.showLoading(false)

                            listener.onSuccess(t?.status)
                        }
                    }))
            }
    }
